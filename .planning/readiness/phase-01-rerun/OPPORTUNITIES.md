...
|---|---|---|---|---|---|---|
| repo-local non-phase external-reread protocol/template | focused cross-model audit integration research | the repo now knows the likely later gap is protocol discipline rather than a new skill, and losing that finding would make later harness follow-through weaker | cleaner later cross-vendor rereads during readiness or post-rerun governance work | stronger durable review discipline without overbuilding the harness | `later-checkpoint` | if Checkpoints 1-2 settle and later harness follow-through still needs a repeatable non-phase external-reread surface |
