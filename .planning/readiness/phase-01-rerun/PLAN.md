...
This checkpoint is conditional.

Run it only if the governance-doc normalization audit concludes that important standing rules are being carried by docs because:

- repo-local GSD workflows are missing lifecycle hooks or translation points
- command/skill surfaces are missing an explicit review or disposition step
- `.planning/config.json` or repo-local workflow defaults materially conflict with the repo's rigor bar
...
Quality gate:
- we are moving rules downward into machinery only where that actually improves reliability
- we are not using automation to hide judgment calls that should remain explicit and reviewable

Checkpoint question:
- is this a real harness defect, or are we trying to use machinery to avoid writing clearer doctrine?

Commit boundary:
- keep harness changes separate from governance-doc wording changes unless the coupling is truly inseparable

### Checkpoint 4: Phase 01 Rerun Readiness Verification
...
Only after Checkpoint 4 is genuinely satisfied:
...
If Checkpoint 1 finds deeper harness ownership problems:
- run Checkpoint 3 before rerun

If Checkpoint 2 improves wording but still leaves the repo relying on ambient operator memory:
- do not declare success; that is evidence for Checkpoint 3

If Checkpoint 4 shows canon still needs another uplift:
...
Immediate next action:
- complete Checkpoint 0 by repairing and re-reviewing the `2026-04-15-multilayer-harness-governance-audit` bundle, then checkpoint the resulting governance/citation wave cleanly

Until Checkpoint 0 closes, do not start the fresh Phase 01 rerun.
