...
- [d:r:i] The typed `v2` first refresh is now landed in `15` plus `artifacts/02-06`, so the active registry baseline is no longer the flat `v1` file by itself.
...
- [d:r:i] The broader propagation question is now also explicit:
  ...
  - the next bounded move is to use the typed `v2` split on the next propagation-relevant change-triggered slice and see which family/edge refresh pressure it actually surfaces
