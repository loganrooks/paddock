...
- [d:r:i] The propagation family now also has an explicit registry-generation policy: future refreshes should be AI-authored from maintained inventory/docs surfaces plus bounded observed/validated carriers, then dispositioned through prose inheritance rather than pretending pure extraction or stale hand-memory can carry the whole semantic field.
...
- [d:r:i] The broader propagation question is now also explicit:
  ...
  - the next bounded move is to revise the registry family before any diff helper: layer the registry more explicitly, split the blended evidence bucket, and refresh the current `v1` slice into a typed/federated `v2` shape against the maintained upstream/local roster frontiers
