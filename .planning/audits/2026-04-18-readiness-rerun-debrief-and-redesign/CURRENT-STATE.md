...
- [d:r:i] The broader propagation question is now also explicit:
  ...
  - the next bounded move is Route C then Route G unless a still-stronger local routing need appears first
...
- [o:r:i] Should the next propagation batch now be the add-vs-overwrite materialization-integrity route (`C` then `G`), or does another still-bounded local chain need to be mapped before that installer-layer move opens?
