...
- current setup/materialization robustness status: `the verifier-lifecycle landing surfaced a still-live fragility: direct local setup-portable-gsd.sh reruns here still fail the upstream gsd-sdk PATH self-check, while portable_gsd_contract.py remains the verified local recovery/materialization path`
