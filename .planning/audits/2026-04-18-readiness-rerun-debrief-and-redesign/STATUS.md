...
4. execute the next narrowed step from [intervention-proposals/12-live-only-agent-cohort-matrix.md](intervention-proposals/12-live-only-agent-cohort-matrix.md): targeted reread on `gsd-debug-session-manager`, then `gsd-pattern-mapper`, before any broader stale-agent cleanup or selected-lane snapshot institutionalization
