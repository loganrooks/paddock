...
- current live-only-agent-next-tranche status: `the remaining live-only agent contracts are still a real intervention surface, but the review/remediation pair is no longer part of the unresolved cohort; gsd-code-reviewer and gsd-code-fixer now carry through tracked overlay/runtime materialization and the next bounded authority/carry review moves to gsd-intel-updater`
- current high-leverage-authority-review status: `the first two bounded authority/carry reviews are now accepted at intervention-proposals/20-gsd-code-reviewer-authority-carry-review.md and intervention-proposals/21-gsd-code-fixer-authority-carry-review.md; current sequence is gsd-intel-updater third, gsd-pattern-mapper fourth`
...
4. run the next bounded authority/carry review on `gsd-intel-updater.toml`, now that the review/remediation pair is carried and the next unresolved leverage surface is the `.planning/intel/` writer
