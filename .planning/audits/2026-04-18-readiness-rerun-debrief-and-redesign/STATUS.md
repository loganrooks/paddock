...
- current live-only-agent-cohort status: `the remaining 16 untracked live-only boundary surfaces are now typed into mostly active routed carry, one weakly routed authority-gap case, and one strong orphan-suspicion case; the next narrowed move is a targeted reread on gsd-debug-session-manager and then gsd-pattern-mapper`
...
- current snapshot-discipline status: `selected-lane runtime snapshot discipline is now accepted; the next strong capture should happen on a clean checkpoint boundary with the new runtime-visibility snapshot wrapper`
- current manifest-install-follow-through status: `manifest/install coherence is now framed as a three-surface problem across manifest truth, backup-meta subset-carry, and selected-lane runtime snapshots rather than any one-file collapse`
...
4. capture the first selected-lane runtime snapshot on the next clean boundary using `python3 tooling/codex/capture_runtime_visibility_snapshot.py`, then use that artifact as the baseline input for the next manifest/install coherence pass
