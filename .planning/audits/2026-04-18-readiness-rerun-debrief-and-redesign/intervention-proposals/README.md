...
1. `01-agent-toml-authority-alignment-proposal.md`
2. `02-launch-truth-capture-proposal.md`
3. `03-manifest-install-coherence-proposal.md`
...
57. `57-transition-lifecycle-carry-first-slice-proposal.md`
...
63. `63-spec-lifecycle-carry-first-slice-proposal.md`
...
85. `85-legacy-seed-corpus-migration-detect-only-first-slice-proposal.md`
86. `86-legacy-seed-corpus-migration-detect-only-first-slice-implementation.md`
87. `87-seed-migration-detect-only-harden-follow-through-proposal.md`
...
99. `99-propagation-review-route-harden-follow-through-proposal.md`
100. `100-propagation-review-route-harden-follow-through-implementation.md`
...
123. `123-milestone-boundary-uplift-shared-reference-first-slice-implementation.md`
124. `124-health-uplift-deepen-in-place-first-slice-proposal.md`
...
151. `151-harness-modifier-helper-payload-authority-map-proposal.md`
...
- [d:r:i] `99` and `100` now record the bounded route-hardening follow-through after the first route reread: durable-note placement, claim-type carry, sharper tool-result-to-disposition guidance, and a slightly wider focused contract-test frontier are now explicit at the same route instead of remaining only in the reread inheritance note.
...
- [d:r:i] `136` now records why that route intensifies: host-project versus harness scope leakage is now real enough that extraction pressure is not only packaging appetite.
- [d:r:i] `169` remains the bounded development-side protocol object, but the sequence no longer stops at proposal plus lane `03`.
- [d:r:i] `170` now keeps verifier/review scheduling explicit as adjacent refinement pressure for development-side parallelization rather than letting it dissolve back into chat memory.
- [d:r:i] `171` now records the landed first development-side protocol slice itself: dominant pattern-library carry, narrow `readiness.md` / `.planning/AGENTS.md` / propagation-review companions, explicit lifecycle declaration shape, and bounded verifier/review scheduling named without widening into a larger automation family.
- [d:r:i] `137` now opens the bounded extraction field map itself: separate generic harness carriers, host-project-specific carriers, and shared boundaries before any repo split or npm packaging route is judged.
...
- [d:r:i] `122` now records the reread-cleared second `119` route in revised form, and `123` now records the landed first slice: milestone-boundary uplift continuity now travels through one dedicated shared read-only reference attached to both milestone workflows, materialized through overlay `add` ownership, with per-boundary surfacing triggers, explicit read-only voice, explicit relation to `60` and `121`, and the matching compatibility-family refresh now carried in `45`.
- [d:r:i] `124` now opens the adjacent single-carrier route left explicit after `123`: `health.md` plus `gsd-health` should deepen in place with one bounded read-only uplift follow-through step rather than widening into another shared-reference family or silently treating repair output as the whole posture story.
...
- [d:r:i] `142` now opens the next extraction object after those two landed slices: classify and separate the generic overlay/workflow/skill/reference tranche that a later standalone harness-modifier project would need, without silently moving host canon or pretending the whole overlay can travel as one blob.
- [d:r:i] `143` now opens the first live review-route follow-through object after the completed widening lane: harden `gsd-review` in place through one helper-backed run-home / launch-truth / timing / salvage layer while keeping `REVIEWS.md` and the planner consumer contract stable.
- [d:r:i] `144` now records the extraction move cleared by lane `02`: freeze the overlay carrier roster plus the embedded-host-path scan first, then use that frozen pair to author the first specialist workflow/skill filesystem rehome proposal.
...
- [d:r:i] `57` now opens the next bounded lifecycle-carry bridge directly: transition should load planning-side `future_preservation`, carry close-boundary judgments explicitly, and keep a compact state digest rather than letting preserved seams and strengthening routes thin at phase close.
...
- [d:r:i] `63` now opens the next bounded upstream lifecycle bridge directly: `spec-phase`, `templates/spec.md`, and the first downstream discuss consumer should keep bounded future-aware carry explicit instead of leaving the spec boundary narrower than the later lifecycle chain.
...
- [d:r:i] `85` now opens the next bounded seed-family move directly: a specialist detect-only migration packet should inventory legacy or drifted seed corpora per seed instead of leaving migration planning at posture counts/examples plus a held-later line.
- [d:r:i] `86` now records the landed detect-only migration slice itself: `seed_migration_inventory.py`, the new workflow and wrapper, and the uplift-route handoff now make per-seed migration planning explicit while keeping direct rewrites separate.
- [d:r:i] `87` now records the bounded harden follow-through earned by the Opus reread over the landed detect-only packet: post-write durable-output state, producer-follow-through to `plant-seed`, route-state disambiguation, uplift-side shape-gap discovery, and narrower reading/write guidance.
...
- [d:r:i] `151` now opens the next extraction object directly: classify helper-payload authority across `project_uplift.py`, `seed_migration_inventory.py`, `audit_refmap.py`, and the current shim boundary before any later payload relocation, default-source-root migration, overwrite-family source split, or standalone-project widening inherits next.
...
- [d:r:i] Lane `10` now closes that reopened `160` judgment on a bounded reread basis: host-planning-shape coupling is discharged, relocation still remains later, the remaining helper coupling now stays explicitly split between install-contract pointer neutralization next and held-later artifact-home movement later.
- [d:r:i] `167` now opens the bounded install-contract pointer neutralization slice directly: move the overlay-manifest path anchor out of helper-local memory before any later payload movement reopens.
- [d:r:i] `168` now keeps the held-later artifact-home question explicit as the sibling after `167`, not as a bundled companion inside the install-contract slice.
- [d:r:i] `169` now opens the next program-wide operational tranche directly: consolidate development-side overlap protocol plus intervention lifecycle review/monitor/iterate discipline, and pair it with the targeted review-route propagation audit instead of leaving the protocol consequences ambient.
...
- [d:r:i] That `165` route is now exercised under its own `parallelization-audit/` subtree: lane `01` is completed through a preserved stalled attempt plus a compact retry, and the current baseline now lives in `parallelization-audit/outputs/01-*` plus `dispositions/01-*` rather than in `165` alone.
- [d:r:i] `161` is now also an opening audit-family route under `responsible-closure-audit/`: the next bounded move is a dedicated field map for deployability, host-context testing, adaptive feedback, discrepancy capture, and harness-agential closure, not another ambient widening of the proposal note alone.
- [d:r:i] `166` is now cross-vendor audited too through `responsible-closure-audit lane-02`: the plan stays sovereign, but now names its inherited taxonomies explicitly, sharpens actual development-surface carriers including the review-workflow family, splits possible development surfaces into held-later-with-trigger / shaped-proposal-not-yet-written / net-virtual, promotes `160` payload-home closure into short horizon, and fixes the next sequence as `160` closure -> development-parallelization plus lifecycle-consolidation protocol tranche -> targeted propagation audit over `145` + `53`.
- [d:r:i] `169` plus `60` are now challenged together through `responsible-closure-audit lane-03`: the pair remains the right bounded object, `60` is now sharpened with concrete candidate under-carried surfaces, the next move is one composite reread over the pair rather than direct implementation, and the expected first protocol slice is now pattern-library-dominant with only narrow `.planning/AGENTS.md` / `readiness.md` carry unless that reread earns more.
