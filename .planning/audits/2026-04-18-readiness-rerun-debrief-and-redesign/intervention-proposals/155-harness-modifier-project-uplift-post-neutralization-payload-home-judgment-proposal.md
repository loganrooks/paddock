...
# Harness Modifier Project Uplift Post-Neutralization Payload-Home Judgment Proposal
...
- [d:r:i] `154` discharged the main helper-local policy blur that had made later movement hard to judge cleanly:
...
- [d:r:i] The slice should judge whether later movement would travel:
  - `project_uplift.py` alone
  - `project_uplift.py` plus a narrower adjacent uplift cluster
  - or no later movement at all because the helper now behaves better as a host-boundary consumer of portable carriers
- [d:r:i] This judgment should keep the already-established downstream relation explicit:
  - `seed_migration_inventory.py` does not lead this decision
  - `harness_canary.py` is a neighboring consumer, not the payload-home authority

### 3. What Is The Cleaner Next Route?

- [d:r:i] If later movement still looks earned, the slice should identify the next bounded preparatory move before any relocation.
- [d:r:i] If later movement no longer looks like the sharper route, the slice should say what family inherits next instead:
  - second overlay tranche
  - overwrite-family source-indirection work
  - standalone repo boundary design
  - or a narrower host/shared-boundary clarification

## Proposed Carrying Surfaces

- [d:r:i] current helper and adjacent consumers:
  - `tooling/codex/project_uplift.py`
  - `tooling/codex/seed_migration_inventory.py`
  - `harness_modifier/contract/harness_canary.py`
- [d:r:i] typed carriers:
...
- [d:r:i] No broader `.codex` / `.claude` parity redesign beyond the already landed carriers.
