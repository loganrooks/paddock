...
- [d:r:i] Lifecycle carry beyond discuss/plan
  - current state: verifier-side first slice landed through `53` and `54`, transition-side first slice landed through `57` and `58`, milestone-boundary first slice landed through `59` and `60`, first-read consumer carry landed through `61` and `62`, spec-boundary carry is now landed through `63` and `64`, the first read-packet / relevance-control slice is now landed through `65` and `66`, and the initialization/doc-ingest inheritance slice is now landed through `67` and `68`
  - current pressure: health / repair, `from-gsd2` / migration, seed consumers, and any later wider packet retrofit beyond the current operator-facing plus initialization/doc-ingest slice
...
- [d:r:i] Lifecycle carry follow-through beyond the current re-entry plus initialization/doc-ingest read-packet slice
