...
**Gathered:** 2026-04-08
**Status:** Revised after roadmap refresh
...
- Stable venue and circuit identifiers must be explicit so `venueRef`, later match snapshots, and pack-level validation do not rely on filename inference or duplicate ID fields.
- Pack round references must use explicit `roundId` plus `path` entries so compile, snapshot, and validation flows do not rely on filename-only ordering or shorthand YAML inference.
...
- How much answer-target lineage should be explicit now for later `section`, `corner`, and composite answers without making v1 authoring meaningfully heavier?
...
*Updated: 2026-04-11 after roadmap refresh reread*
