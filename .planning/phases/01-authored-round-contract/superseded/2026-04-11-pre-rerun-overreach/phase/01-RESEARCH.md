...
**Confidence:** HIGH
**Refresh note:** 2026-04-11 canon refresh kept this research directionally valid, but Phase 1 execution now explicitly freezes canonical venue identity, exact pack round-reference shape, and stable validation diagnostics before implementation starts.
...
- **D-03:** Keep the active v1 answer-surface contract anchored at `circuit` and `venue`, while preserving explicit `venue -> circuit -> section -> corner` lineage so later deeper targets do not require a flat-contract rewrite.
...
The highest-leverage modeling choices are to keep semantic clue family separate from media transport, to preserve explicit answer-target lineage, to freeze canonical venue and pack-reference identities early, and to make venue coverage plus fallback posture explicit data instead of hidden author judgment. That directly aligns with PACK-03, PACK-04, OPS-01, the discovery coverage audit, and the phase constraint that fallback-heavy venues must be explicit instead of silent substitutions. [VERIFIED: .planning/REQUIREMENTS.md][VERIFIED: .planning/phases/01-authored-round-contract/01-CONTEXT.md][VERIFIED: discovery/11-circuit-coverage-audit.md]

**Primary recommendation:** Use declarative YAML source files under `content/`, validate them with Zod schemas in `packages/domain`, compile them with `packages/content-tools`, and model `answerTarget.lineage`, `clueFamily`, `media.kind`, `venueCoverage`, and `fallbackStrategy` as separate first-class fields. Treat venue profile `id` as the canonical `venueRef` target, and freeze pack `roundOrder[]` entries as exact `{ roundId, path }` objects before fixture authoring begins. [CITED: https://eemeli.org/yaml/][CITED: https://zod.dev/v4?id=introducing-zod-mini][VERIFIED: .planning/phases/01-authored-round-contract/01-CONTEXT.md]
...
**What:** Model `clueFamily` independently from `media.kind` so a `circuit_internal` clue can be delivered by Street View, image, `map_fragment`, or text without changing the round contract. [VERIFIED: .planning/phases/01-authored-round-contract/01-CONTEXT.md]
...
**Refreshed against canon:** 2026-04-11
