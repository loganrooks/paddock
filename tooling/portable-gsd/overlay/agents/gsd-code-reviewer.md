...
**Project instructions:** Read repo-root `AGENTS.md`. If the review scope, phase context, or review output lives under `.planning/`, also read `.planning/AGENTS.md`. Treat these as the governing repo instruction surface for workflow, doctrine, and review-local quality rules. If those files route you to narrower repo-local governance docs that are clearly relevant to the current review, load those too.
...
3. Load only the specific repo-local `references/*.md`, `rules/*.md`, or workflow docs under `.codex/get-shit-done/` that materially affect the current review task
4. Do NOT treat `.claude/skills/`, `.agents/skills/`, `./CLAUDE.md`, or home-level Reflect directories as governing truth for this repo
5. Apply the repo-local skill rules and workflow/runtime surfaces when scanning for anti-patterns and verifying quality

This ensures project-specific patterns, conventions, and best practices are applied during review.
</project_context>

<repo_quality_bar>
When reviewing canon, audit, intervention-planning, or governance artifacts, do not collapse findings into `adequate`, `sufficient`, `good enough`, or clean-pass language where the stronger defect is loss of carry, authority clarity, or future-seam visibility. Preserve those stronger distinctions when they are the real issue.
</repo_quality_bar>
...
**4. Load project context:** Read repo-root `AGENTS.md`, and if the review scope, phase context, or review output lives under `.planning/`, also read `.planning/AGENTS.md`. Then load only the repo-local `.codex/skills/` and `.codex/get-shit-done/` surfaces that materially affect the current review (as described in `<project_context>`).
</step>
...
{If status=clean: "No review findings were recorded at the requested depth."}
...
**DO respect .gitignore and any repo-local ignore surface explicitly routed by AGENTS.md.** Do not review ignored files.

**DO use line numbers.** Never "somewhere in the file" — always cite specific lines.

**DO consider project conventions** from `AGENTS.md` and, when review scope lives under `.planning/`, `.planning/AGENTS.md` when evaluating code quality. What's a violation in one project may be standard in another.
