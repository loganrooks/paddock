...
**Project instructions:** Read repo-root `AGENTS.md`. Because this agent writes `.planning/intel/`, also read `.planning/AGENTS.md`. Treat those as the governing repo instruction surface for workflow, doctrine, and intel-local quality rules. If they route you to narrower repo-local governance docs that are clearly relevant to the current intel task, load those too.
...
3. Load only the specific repo-local `references/*.md`, `rules/*.md`, workflow docs, or compact-prompt docs under `.codex/` that materially affect the current intel task
4. Do NOT treat `.claude/skills/`, `.agents/skills/`, `./CLAUDE.md`, or home-level Reflect directories as governing truth for this repo
5. Apply repo-local skill rules and runtime topology facts so intel outputs reflect actual carried harness surfaces, not legacy layout assumptions
...
<repo_quality_bar>
When writing intel that later agents will rely on, do not collapse runtime-authoritative versus paired human-facing surfaces, or broader carry/authority distinctions, into threshold language like `good enough`, `adequate`, or `close enough`. Write the stronger distinction when it affects later planning or intervention yield.
</repo_quality_bar>
...
**Runtime layout for this repo (do this first):** Use the repo-local Codex GSD layout rooted at `.codex/`.

Resolve canonical harness paths from that root:

| Source type | `.codex` layout |
|-------------|-----------------|
| Runtime-authoritative agent files | `.codex/agents/*.toml` |
| Paired human-facing agent files | `.codex/agents/*.md` |
| Skill files | `.codex/skills/*/SKILL.md` |
| CLI tooling | `.codex/get-shit-done/bin/` |
| Workflow files | `.codex/get-shit-done/workflows/` |
| Reference docs | `.codex/get-shit-done/references/` |
| Compact prompts | `.codex/tooling/compact-prompts/*.md` |

When analyzing repo-local GSD surfaces in this project, use these `.codex` locations as canonical truth. Do not invent `.claude` or `.kilo` roots for this repo.
...
counts by running Glob on the `.codex` canonical locations above, not from memory or AGENTS.md.
Examples: `Glob(".codex/agents/*.toml")`, `Glob(".codex/agents/*.md")`, `Glob(".codex/get-shit-done/workflows/*.md")`.
