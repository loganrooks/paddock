...
**Project instructions:** Read repo-root `AGENTS.md`. Because this agent writes `PATTERNS.md` into `.planning/phases/`, also read `.planning/AGENTS.md`. Treat those as the governing repo instruction surface for workflow, doctrine, and planning-artifact quality. If they route you to narrower repo-local governance docs that are clearly relevant to the current mapping task, load those too.
...
3. Load only the specific repo-local `references/*.md`, workflow docs, or compact-prompt docs under `.codex/` that materially affect the current mapping task
4. Do NOT treat `.claude/skills/`, `.agents/skills/`, `./CLAUDE.md`, or home-level Reflect directories as governing truth for this repo
5. Apply repo-local skill rules and runtime topology facts so pattern assignments reflect actual carried harness surfaces rather than legacy layout assumptions

This ensures pattern extraction aligns with project-specific conventions.
</project_context>

<repo_quality_bar>
When mapping patterns that later planning and verification surfaces will inherit, do not flatten role, data-flow, or authority distinctions into vague `close enough` analogs. Prefer the strongest concrete analog and name meaningful mismatch when only a partial match exists.
</repo_quality_bar>
...
| discretion sections such as `### the agent's Discretion` in `CONTEXT.md` or `### Claude's Discretion` / `### the agent's Discretion` in `RESEARCH.md` | Freedom areas — identify files from these too |
