...
**Project instructions:** Read repo-root `AGENTS.md`. If the REVIEW.md scope, phase context, fix report, or touched files live under `.planning/`, also read `.planning/AGENTS.md`. Treat these as the governing repo instruction surface for workflow, doctrine, and fix-local quality rules. If those files route you to narrower repo-local governance docs that are clearly relevant to the current fix task, load those too.

**Project skills/runtime:** Use the repo-local Codex surfaces that actually govern this repo:
1. Check `.codex/skills/` for repo-local skills when skill patterns are relevant
2. Read only the needed `SKILL.md` files under `.codex/skills/`
3. Load only the specific repo-local `references/*.md`, `rules/*.md`, or workflow docs under `.codex/get-shit-done/` that materially affect the current fix task
4. Do NOT treat `.claude/skills/`, `.agents/skills/`, `./CLAUDE.md`, or home-level Reflect directories as governing truth for this repo
5. Apply the repo-local skill rules and workflow/runtime surfaces when adapting fixes and deciding whether a rollback or commit path is safe

This ensures project-specific patterns, conventions, and best practices are applied during fixes.
</project_context>

<repo_quality_bar>
When fixing canon, audit, intervention-planning, or governance artifacts, preserve stronger carry, authority, and future-seam distinctions instead of flattening the result into threshold language like `adequate`, `sufficient`, `good enough`, or `ready` unless the task is a true narrow gate.
</repo_quality_bar>
...
2. **Capture rollback state before editing:** Before the first edit to each existing file, copy it into a per-finding rollback directory. If the fix must create a new file, record that file path separately as `created_during_fix`.

3. **Apply fix:** Use Edit tool (preferred) for targeted changes.

4. **Verify fix:** Apply 3-tier verification strategy (see verification_strategy).

5. **On verification failure:**
   - Restore each previously existing file from its captured rollback copy.
   - Remove only files created during the failed finding.
   - Do NOT use `git checkout --`, `git restore`, or blanket working-tree reset commands for per-finding rollback in this repo.
...
**Rollback scope:** Per-finding only. Restore only the files captured for the current finding and remove only files created during that finding. Files modified by prior (already committed) findings are NOT touched during rollback.
...
Read repo-root `AGENTS.md`. If REVIEW.md, `phase_dir`, `fix_report_path`, or the touched files live under `.planning/`, also read `.planning/AGENTS.md`. Then load only the repo-local `.codex/skills/` and `.codex/get-shit-done/` surfaces that materially affect the current fix task (as described in `<project_context>`).
</step>
...
- For EVERY file about to be modified:
  ...
  - If the file already exists, capture a rollback copy before the first edit for this finding
  - If the fix must create the file, record it in `created_during_fix`
  - Do NOT rely on git working-tree reset commands for rollback in this repo
...
**DO record which files will be touched** before every fix attempt and capture rollback copies before the first edit to each existing file.
...
**DO rollback from captured pre-fix state** and remove only files created during the failed finding. Do NOT use `git checkout --`, `git restore`, or blanket working-tree reset commands for rollback in this repo.
...
**DO respect repo-root `AGENTS.md` project conventions** during fixes, and if the fix scope or outputs live under `.planning/`, also respect `.planning/AGENTS.md`. If the repo requires specific patterns (e.g., no `any` types, specific error handling), apply them.
...
- [ ] No source files left in broken state (failed fixes restored from captured rollback state)
...
- [ ] Safe rollback restored captured pre-fix state without using git working-tree reset commands
- [ ] Skipped findings documented with specific skip reasons
- [ ] Project conventions from `AGENTS.md` (and `.planning/AGENTS.md` when relevant) respected during fixes
