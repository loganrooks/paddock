...
@__PROJECT_ROOT__/.codex/get-shit-done/references/ui-brand.md
@__PROJECT_ROOT__/.codex/get-shit-done/references/revision-loop.md
...
```bash
INIT=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
CONTEXT_WINDOW=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" config-get context_window 2>/dev/null || echo "200000")
```
...
5. Commit:
```bash
node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
[If DISCUSS_MODE is "exploratory" or "discuss" or unset:]
2. Run discuss-phase first — Capture a phase steering brief before planning
...
- header: "No context"
- question: "No CONTEXT.md found for Phase {X}. Plans will use research and requirements only — the steering brief (decisions, assumptions, open questions, future-awareness constraints) won't be included. Continue or capture context first?"
- options:
  - "Continue without context" — Plan using research + requirements only
  If `DISCUSS_MODE` is `"assumptions"`:
  - "Gather context (assumptions mode)" — Analyze codebase and surface assumptions before planning
  If `DISCUSS_MODE` is `"exploratory"` or `"discuss"` (or unset):
  - "Run discuss-phase first" — Capture a phase steering brief before planning
...
```bash
PHASE_DESC=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `__PROJECT_ROOT__/.codex/get-shit-done/templates/VALIDATION.md`
...
```bash
SECURITY_CFG=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
SECURITY_ASVS=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```bash
PHASE_SECTION=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
2. Disable Nyquist with the exact command:
   `node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
```bash
node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
