...
- Count PLAN files
- Count SUMMARY files
- If counts match: execution is complete
...
OUTSTANDING=""
for f in .planning/phases/XX-current/*-SUMMARY.md .planning/phases/XX-current/*-UAT.md .planning/phases/XX-current/*-VERIFICATION.md; do
  [ -f "$f" ] || continue
  grep -q "^## Self-Check: FAILED\|^completion_mode: debt_carrying_execution\|^completion_mode: debt_carrying_completion\|^completion_debt:\|result: pending\|result: blocked\|status: testing\|status: partial\|status: human_needed\|status: gaps_found\|status: diagnosed" "$f" && OUTSTANDING="$OUTSTANDING\n$(basename $f)"
done
```
...
This is a debt-carrying transition, not clean completion. Review: `$gsd-audit-uat`
```

Debt does not silently block transition, but it DOES change the confirmation semantics and the recorded completion mode.
Summary frontmatter debt fields count here too; do not treat a present SUMMARY as clean completion when it explicitly records `debt_carrying_execution` or structured `completion_debt`.

**If all plans complete:**

<if mode="yolo" AND="OUTSTANDING is empty">

```
⚡ Auto-approved: Transition Phase [X] → Phase [X+1]
Phase [X] complete — all [Y] plans finished cleanly.
...
<if mode="interactive" OR="custom with gates.confirm_transition true" AND="OUTSTANDING is empty">

Ask: "Phase [X] complete — all [Y] plans finished cleanly. Ready to mark done and move to Phase [X+1]?"

Wait for confirmation before proceeding.

</if>

**If all plans complete AND OUTSTANDING is not empty:**

Treat this as a debt-carrying completion decision. Always confirm explicitly, even in yolo/custom modes.

Present:

```
Phase [X] finished execution, but completion debt is still open:
{list filenames}

Transitioning now will record debt-carrying completion, not clean completion.

Options:
1. Continue current phase and resolve the debt first
2. Advance anyway and carry the debt forward explicitly
3. Review open verification debt
```
...
```bash
TRANSITION=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```

The CLI handles:
- Marking clean completion with a `[x]` checkbox and `Complete` status
- Recording non-clean completion as `Executed` or `Debt Carry` without silently promoting it to clean completion
- Updating plan count text to `plans complete` only for clean completion; otherwise `plans executed`
- Updating the Progress table with the debt-aware status and clean-completion date only when appropriate
...
Extract from result: `completed_phase`, `plans_executed`, `completion_mode`, `clean_completion`, `next_phase`, `next_phase_name`, `is_last_phase`.
...
```bash
PROGRESS=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
