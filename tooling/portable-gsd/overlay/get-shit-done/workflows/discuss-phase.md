<purpose>
Create `CONTEXT.md` as a phase steering brief that downstream research and planning can rely on.

You are not just interviewing for preferences. Your job is to reduce avoidable ambiguity before planning by capturing boundary, assumptions, locked decisions, derived constraints, open questions, epistemic guardrails, discretion areas, future awareness, and deferred ideas.

You are a thinking partner, not an interviewer. The user is the visionary — you are the builder. Your job is to create the steering artifact those later stages need, not to do full research or planning here.
</purpose>

<required_reading>
@__PROJECT_ROOT__/.codex/get-shit-done/references/domain-probes.md
@__PROJECT_ROOT__/.codex/get-shit-done/references/gate-prompts.md
@__PROJECT_ROOT__/.codex/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
- constraints derived from requirements, prior phases, or code reality
- open questions that should steer research
- epistemic guardrails and verification standards
- future awareness constraints that should shape current implementation
...
- **Working assumptions, not fake certainty:** Frame unresolved choices as current assumptions or open questions instead of forcing "locked" decisions.
- **Epistemic guardrails:** Note which options are grounded by codebase patterns, prior decisions, or established conventions versus which remain open or speculative.
- **Preserve genuine uncertainty:** If the user is unsure or multiple approaches remain viable, record that honestly for downstream agents.
- **Auto-select grounding rule:** In `--auto`, only auto-select when strongly grounded. Otherwise record an open question for research/planning.
</philosophy>
...
Required sections: `domain`, `decisions`, `canonical_refs`, `code_context`, `specifics`, `future_awareness`, `deferred`.
Only include the uncertainty-aware sections when they have real content.
</context_model>
...
AGENT_SKILLS_ADVISOR=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-advisor 2>/dev/null)
```
...
Display status:
- `exploratory` → `Discuss mode: exploratory — preserving uncertainty, auto-selecting only when grounded.`
- `discuss` → `Discuss mode: discuss — capturing standard steering decisions.`
- `assumptions` → handled by skill routing before this workflow begins
...
- Read and execute @__PROJECT_ROOT__/.codex/get-shit-done/workflows/discuss-phase-power.md end-to-end
...
- In `discuss_areas`:
  - if `DISCUSS_MODE` is `discuss`, choose the recommended option (first option, or the one marked "recommended")
  - if `DISCUSS_MODE` is `exploratory`, choose only strongly grounded options; otherwise record the question as intentionally open
...
5. **Exploratory synthesis posture** — If `DISCUSS_MODE` is `exploratory`, bias your analysis toward:
   - what is genuinely grounded by prior decisions or code reality
   - what should remain a working assumption
   - what should become an open question for research
   - what deserves an epistemic guardrail because evidence is weak or multiple approaches remain valid
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-advisor-researcher --raw)
   ```
...
**Exploratory mode annotation:** When `DISCUSS_MODE` is `exploratory`, annotate where a direction is `[grounded]` versus `[open]` based on codebase precedent, prior decisions, or explicit requirements. Do not fake grounding where none exists.
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @__PROJECT_ROOT__/.codex/agents/gsd-advisor-researcher.md for your role and instructions.
...
**Auto mode (`--auto`):**
- In `discuss` mode: select the recommended option (first option, or the one explicitly marked "recommended") for every question without using AskUserQuestion.
- In `exploratory` mode: only auto-select options that are strongly grounded by codebase patterns, prior decisions, or explicit requirements. For ungrounded cases, record an open question instead of forcing a decision.
...
After all areas are auto-resolved or intentionally left open, skip the "Explore more gray areas" prompt and proceed directly to write_context.
...
```bash
MAX_PASSES=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
[Use when `DISCUSS_MODE` is exploratory or when legitimate uncertainty remains.
Record current assumptions that should guide downstream work but remain open to revision.]
...
[Constraints inferred from requirements, prior phases, architecture, operations, or external dependencies.
These are not user preferences; they shape what viable implementations look like.]
...
[High-leverage ambiguities intentionally left open for research or planning to resolve.
In exploratory `--auto`, this is where ungrounded questions should land instead of being forced into fake decisions.]
...
[How downstream agents should treat uncertainty in this phase.
Note where evidence is weak, what must be validated, and which assumptions should not be treated as settled truth.]
...
```bash
node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
**Full step-by-step instructions:** @__PROJECT_ROOT__/.codex/get-shit-done/workflows/discuss-phase-power.md
...
- `exploratory` mode preserves legitimate uncertainty instead of forcing false precision
</success_criteria>
