...
Produces a SPEC.md with falsifiable requirements and, when doctrine materially constrains the phase, bounded `Future-Aware Notes` that discuss-phase treats as upstream steering.
...
- `.planning/LONG-ARC.md` when it exists — durable doctrine that should shape bounded future-aware carry
...
- Whether long-arc doctrine or the next few downstream phases create bounded future-aware pressure that current requirements should not erase
...
**Derive bounded future-aware notes** — only when the repo doctrine materially constrains the phase. Use these buckets:
- `Protected Seams`
...
These notes must shape HOW the current phase is specified without turning later scope into current scope.
...
**If `--auto` and initial ambiguity already ≤ 0.20 with all minimums met:** Skip interview — derive SPEC.md directly from existing context. Log: `[auto] Phase requirements are already clear from roadmap, requirements, and current doctrine — generating SPEC.md from existing context.` Jump to Step 6.
...
- question: "Ambiguity is [score] — the requirement contract is now explicit across goal, boundary, constraint, and acceptance. Proceed to SPEC.md?"
- options:
...
Use the SPEC.md template from @__PROJECT_ROOT__/.codex/get-shit-done/templates/spec.md.
...
**If bounded future-aware notes are materially present**, write the optional `## Future-Aware Notes` section using these buckets:
- `Protected Seams`
- `Explicit Non-Decisions`
- `Current Posture`
- `Future Shape Notes`
...
Keep those notes narrow:
- they shape how the current phase should be specified
- they do not add new deliverables to current scope
- they should be omitted entirely when no bounded future-aware pressure is present
...
Next: $gsd-discuss-phase {X}
  discuss-phase will read SPEC.md as upstream steering and focus on implementation decisions.
```
...
- `Future-Aware Notes` must never be used to smuggle later scope into the current phase
</critical_rules>
...
- When doctrine materially constrains the phase, SPEC.md includes bounded `Future-Aware Notes`
