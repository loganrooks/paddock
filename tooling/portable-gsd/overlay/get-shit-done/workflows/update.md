...
<deeper_reading>
Only widen into structural planning carriers, governing docs, or broader doctrine posture once the runtime/package boundary is already understood and the route actually shifts away from update into a different owned surface. Do not blend runtime install/version change, structural repair, and broader project uplift into one pass.
</deeper_reading>
...
You're already on the latest runtime/package version.

If the live issue is structural planning integrity, run: $gsd-health
If runtime/package state is current but repo-local posture still deserves refresh, run: $gsd-uplift-project --write
```
...
Keep update ownership explicit:

- `update` owns runtime/package change, installer rerun, clean-install warning, custom-file backup, and patch recovery reminders.
...
If the remaining issue is structural planning integrity, run: $gsd-health
If runtime/package movement is handled but repo-local posture still deserves refresh, run: $gsd-uplift-project --write
