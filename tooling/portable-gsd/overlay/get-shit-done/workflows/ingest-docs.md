...
<supporting_reading>
Start from the narrow ingest packet this workflow already owns:
- parsed arguments plus `init.ingest-docs` JSON
...
<deeper_reading>
Only widen into broader existing planning canon, runtime/governance carriers, or later uplift material when the merge route, conflict surface, or later follow-through explicitly points there. Do not flatten doc discovery, merge planning, and repo-local uplift into one startup burden.
</deeper_reading>
...
Keep the ingest packet layered:
...
This workflow merges planning docs. If repo-local runtime or governing carriers also need refresh after importing older or vanilla planning state, route separately to `$gsd-uplift-project --write` rather than folding that extra family into ingest itself.
