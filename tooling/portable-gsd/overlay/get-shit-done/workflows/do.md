...
```bash
INIT=$(node "__PROJECT_ROOT__/.codex/get-shit-done/bin/gsd-tools.cjs" state load 2>/dev/null)
```
...
| Reviewing a phase plan with external AIs, peer-reviewing plans, or asking for plan critique before execution | `$gsd-review` | Cross-AI plan review, not execution verification |
| Replanning after REVIEWS.md feedback, "apply review feedback to the plan", or "replan from plan review comments" | `$gsd-plan-phase --reviews` | Fresh planning pass that must consume review feedback explicitly |
...
| A review or quality concern about existing implemented work | `$gsd-verify-work` | Needs verification of work that already landed |
...
If the chosen route includes preset flags such as `--reviews`, preserve them in the dispatched command instead of dropping back to the base skill.
