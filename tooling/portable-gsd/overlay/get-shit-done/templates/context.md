...
Template for `.planning/phases/XX-name/{phase_num}-CONTEXT.md` - captures the phase steering brief used by downstream agents.

**Purpose:** Document the steering context that downstream agents need. Researcher uses this to know WHAT to investigate. Planner uses this to know WHAT choices are locked, which assumptions are provisional, which constraints come from future phases, and which questions remain open.

**Key principle:** Categories are NOT predefined. They emerge from what was actually discussed for THIS phase. A CLI phase has CLI-relevant sections, a UI phase has UI-relevant sections.

**Downstream consumers:**
- `gsd-phase-researcher` — Reads decisions, assumptions, open questions, and future-awareness constraints to focus research
- `gsd-planner` — Reads decisions, constraints, and future-awareness guardrails to create specific tasks without closing doors for later phases
...
[Use when the phase still contains legitimate uncertainty. Record current assumptions that should guide research/planning but may be revised with evidence.]
...
[Constraints inferred from requirements, prior phases, architecture, operations, or external systems. These are not taste/preferences; they shape what viable implementations look like.]
...
[High-leverage ambiguities that remain intentionally open and should steer downstream research or planning.]
...
[How downstream agents should treat uncertainty in this phase. Note where evidence is weak, where choices are provisional, or what must be validated before implementation is considered correct.]
...
Examples below illustrate the tone and level of specificity. They may omit some newer steering-brief sections for brevity. In the current workflow, always include the required sections listed above, especially `canonical_refs` and `future_awareness`.
...
<guidelines>
**This template captures a STEERING BRIEF for downstream agents.**

The output should answer:
- What does the researcher need to investigate?
- What choices are locked for the planner?
- What assumptions are provisional?
- What future constraints should shape implementation now?
...
- `gsd-phase-researcher` uses decisions, open questions, and future awareness to focus investigation AND reads canonical_refs to know WHAT docs to study
- `gsd-planner` uses decisions, constraints, future awareness, and research to create executable tasks AND reads canonical_refs to verify alignment
