...
**Purpose:** Capture WHAT a phase delivers and WHY, with enough precision that requirements are falsifiable. When long-arc doctrine materially constrains the phase, the spec can also carry bounded `Future-Aware Notes` so discuss-phase does not silently narrow the chain later.
...
- `discuss-phase` — reads SPEC.md at startup; treats Requirements, Boundaries, Acceptance Criteria, and `Future-Aware Notes` as upstream steering; focuses on HOW rather than reopening locked WHAT/WHY
- `gsd-planner` — reads locked requirements through the discuss/context chain to constrain plan scope
...
[Optional — include only when long-arc doctrine, downstream phases, or bounded strengthening pressure materially constrain the current phase. This section shapes how the current phase should be specified. It does not add later-scope deliverables to the current phase.]

### Protected Seams
- [Interface, abstraction, schema, or authored-shape boundary that current work must preserve]

### Explicit Non-Decisions
- [Choice that should stay intentionally open in this phase rather than being silently locked]

### Current Posture
- [Current trust, visibility, or service-obligation stance that downstream work must respect]

### Future Shape Notes
- [Constrained note about plausible wrapper, sibling surface, or later product shape that should influence present seams without pulling that scope forward]

### Strengthening Opportunities
- [Bounded move that intensifies already-useful terrain, preserves more optionality, or should route into current work or durable later resurfacing]

[If no bounded future-aware pressure is present, omit this section entirely.]
...
**Future-Aware Notes keep the upstream contract from narrowing later carry.** When present, they record the bounded seams, non-decisions, posture, future-shape pressure, and strengthening routes that should shape the current phase without pulling later scope into it.

**SPEC.md is a one-way door for requirements.** discuss-phase will treat these as upstream steering. If requirements change after SPEC.md is written, the user should update SPEC.md first, then re-run discuss-phase.

**SPEC.md does NOT replace CONTEXT.md.** They serve different purposes:
- SPEC.md: what the phase delivers (requirements, boundaries, acceptance criteria, and bounded future-aware notes when needed)
