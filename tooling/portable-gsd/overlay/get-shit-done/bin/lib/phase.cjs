...
const CLEAN_COMPLETION = 'clean_completion';
const DEBT_CARRYING_COMPLETION = 'debt_carrying_completion';
...
function completionModeToRoadmapStatus(completionMode) {
  switch (completionMode) {
    case CLEAN_COMPLETION:
      return 'Complete';
    case DEBT_CARRYING_COMPLETION:
      return 'Debt Carry';
...
function completionModeUsesCompleteLanguage(completionMode) {
  return completionMode === CLEAN_COMPLETION;
}

function inspectPhaseCompletion(phaseDir) {
  const result = {
    verification_present: false,
    verification_status: null,
    verification_completion_mode: null,
    overrides_applied: 0,
    debt_bearing: false,
    completion_mode: VERIFICATION_PENDING,
    warnings: [],
  };
  ...
  const verificationPriority = { gaps_found: 3, human_needed: 2, passed: 1 };
  let bestVerificationPriority = -1;

  for (const file of files.filter(f => f.endsWith('-SUMMARY.md') || f === 'SUMMARY.md')) {
    const content = fs.readFileSync(path.join(phaseDir, file), 'utf-8');
    if (/^##\s*Self-Check:\s*FAILED\b/im.test(content)) {
      pushUnique(result.warnings, `${file}: summary self-check failed`);
    }
  }
  ...
  for (const file of files.filter(f => f.includes('-UAT') && f.endsWith('.md'))) {
    ...
    if (status === 'testing') pushUnique(result.warnings, `${file}: testing still in progress`);
    if (status === 'partial') pushUnique(result.warnings, `${file}: testing incomplete (partial)`);
    if (status === 'diagnosed') pushUnique(result.warnings, `${file}: has diagnosed gaps`);
    if (/result:\s*pending\b/i.test(content)) pushUnique(result.warnings, `${file}: has pending tests`);
    if (/result:\s*blocked\b/i.test(content)) pushUnique(result.warnings, `${file}: has blocked tests`);
  }
  ...
  for (const file of files.filter(f => f.includes('-VERIFICATION') && f.endsWith('.md'))) {
    ...
    const frontmatter = extractFrontmatter(content);
    const status = String(frontmatter.status || '').toLowerCase();
    const completionMode = String(frontmatter.completion_mode || '').toLowerCase();
    const debtBearing = String(frontmatter.debt_bearing || '').toLowerCase() === 'true';
    const overridesApplied = parseInt(frontmatter.overrides_applied, 10) || 0;

    result.verification_present = true;

    const priority = verificationPriority[status] || 0;
    if (priority > bestVerificationPriority) {
      bestVerificationPriority = priority;
      result.verification_status = status || null;
      result.verification_completion_mode = completionMode || null;
      result.overrides_applied = overridesApplied;
    } else if (priority === bestVerificationPriority) {
      result.overrides_applied = Math.max(result.overrides_applied, overridesApplied);
    }

    if (status === 'human_needed') pushUnique(result.warnings, `${file}: needs human verification`);
    if (status === 'gaps_found') pushUnique(result.warnings, `${file}: has unresolved gaps`);

    if (debtBearing || completionMode === DEBT_CARRYING_COMPLETION || overridesApplied > 0) {
      result.debt_bearing = true;
    }
  }

  if (result.warnings.length > 0 || result.debt_bearing) {
    result.debt_bearing = true;
    result.completion_mode = DEBT_CARRYING_COMPLETION;
  } else if (result.verification_present && result.verification_status === 'passed') {
    result.completion_mode = CLEAN_COMPLETION;
  }
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  const completion = inspectPhaseCompletion(phaseFullDir);
  const completionMode = completion.completion_mode;
  const roadmapStatus = completionModeToRoadmapStatus(completionMode);
  const shouldMarkRoadmapComplete = completionModeUsesCompleteLanguage(completionMode);
    ...
    withPlanningLock(cwd, () => {
      ...
      roadmapContent = roadmapContent.replace(tableRowPattern, (fullRow) => {
        ...
        if (cells.length === 5) {
          ...
          cells[3] = ` ${roadmapStatus.padEnd(11)}`;
          cells[4] = shouldMarkRoadmapComplete ? ` ${today} ` : '  ';
        } else if (cells.length === 4) {
          // 4-col: Phase | Plans | Status | Completed
          cells[1] = ` ${summaryCount}/${planCount} `;
          cells[2] = ` ${roadmapStatus.padEnd(11)}`;
          cells[3] = shouldMarkRoadmapComplete ? ` ${today} ` : '  ';
        }
        return '|' + cells.join('|') + '|';
      });
      ...
      roadmapContent = roadmapContent.replace(
        planCountPattern,
        `$1${summaryCount}/${planCount} plans ${shouldMarkRoadmapComplete ? 'complete' : 'executed'}`
      );

      if (shouldMarkRoadmapComplete) {
  ...
  if (fs.existsSync(statePath)) {
    readModifyWriteStateMd(statePath, (stateContent) => {
      ...
      // Update Status
      const nextStatus = shouldMarkRoadmapComplete
        ? (isLastPhase ? 'Milestone complete' : 'Ready to plan')
        : (isLastPhase
            ? `Milestone complete pending debt review from Phase ${phaseNum}`
            : `Ready to plan (carried debt from Phase ${phaseNum})`);
      stateContent = stateReplaceFieldWithFallback(stateContent, 'Status', null, nextStatus);
      ...
      stateContent = stateReplaceFieldWithFallback(stateContent, 'Last Activity Description', null,
        `Phase ${phaseNum} ${shouldMarkRoadmapComplete ? 'completed cleanly' : 'recorded with carried debt'}${nextPhaseNum ? `, transitioned to Phase ${nextPhaseNum}` : ''}`);
  ...
  const result = {
    ...
    clean_completion: shouldMarkRoadmapComplete,
    debt_bearing: completion.debt_bearing,
...
module.exports = {
  CLEAN_COMPLETION,
  DEBT_CARRYING_COMPLETION,
  VERIFICATION_PENDING,
  completionModeToRoadmapStatus,
  completionModeUsesCompleteLanguage,
  inspectPhaseCompletion,
  cmdPhasesList,
  cmdPhaseNextDecimal,
  cmdFindPhase,
  cmdPhasePlanIndex,
