...
function extractBulletItems(sectionContent) {
  ...
  return (sectionContent.match(/^-\s+(.+)$/gm) || [])
    .map((item) => item.replace(/^-\s+/, '').trim())
    .filter(Boolean);
}

function extractDecisions(content) {
  const section = extractMarkdownSection(content, '(?:Decisions|Decisions Made|Accumulated.*Decisions)');
  if (!section) return [];
  ...
  return extractBulletItems(section).map((decision) => ({ decision }));
}

function extractBlockers(content) {
  const section = extractMarkdownSection(content, '(?:Blockers|Blockers\\/Concerns|Blockers\\s*&\\s*Concerns|Concerns)');
  if (!section) return [];
  return extractBulletItems(section).map((text) => ({ text }));
}

function extractFutureCarry(content) {
  const section = extractMarkdownSection(content, 'Future Carry Forward');
  const result = {
    preserve: [],
    keep_open: [],
    posture: [],
  ...
  for (const line of lines) {
    let match = line.match(/^Preserve:\s*(.+)$/i);
    if (match) {
      result.preserve.push(match[1].trim());
      continue;
    }
    match = line.match(/^Keep open:\s*(.+)$/i);
    if (match) {
      result.keep_open.push(match[1].trim());
      continue;
    }
    match = line.match(/^Posture:\s*(.+)$/i);
    if (match) {
      result.posture.push(match[1].trim());
      continue;
    }
    match = line.match(/^Seeded:\s*(.+)$/i);
    if (match) {
      result.seeded.push(match[1].trim());
    }
  }
...
function extractSessionContinuity(content) {
  const session = {
    last_date: null,
    last_session: null,
    stopped_at: null,
  ...
  const section = extractMarkdownSection(content, 'Session(?: Continuity)?');
  if (!section) return session;

  const lastDateMatch = section.match(/\*\*(?:Last Date|Last session):\*\*\s*(.+)/i)
    || section.match(/^(?:Last Date|Last session):\s*(.+)$/im);
  const stoppedAtMatch = section.match(/\*\*Stopped (?:At|at):\*\*\s*(.+)/i)
    || section.match(/^Stopped (?:At|at):\s*(.+)$/im);
  const resumeFileMatch = section.match(/\*\*Resume File:\*\*\s*(.+)/i)
    || section.match(/^Resume File:\s*(.+)$/im);

  if (lastDateMatch) {
    session.last_date = lastDateMatch[1].trim();
    session.last_session = lastDateMatch[1].trim();
  }
...
function cmdStateSnapshot(cwd, raw) {
  ...
  const futureCarry = extractFutureCarry(content);
  const session = extractSessionContinuity(content);
