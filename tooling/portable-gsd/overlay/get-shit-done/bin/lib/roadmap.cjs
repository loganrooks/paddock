...
const { CLEAN_COMPLETION, DEBT_CARRYING_COMPLETION, completionModeToRoadmapStatus, completionModeUsesCompleteLanguage, inspectPhaseCompletion } = require('./phase.cjs');
...
function cmdRoadmapAnalyze(cwd, raw) {
  ...
  while ((match = phasePattern.exec(content)) !== null) {
    ...
    try {
      ...
      if (dirMatch) {
        ...
        if (summaryCount >= planCount && planCount > 0) {
          const completion = inspectPhaseCompletion(phaseDir);
          completionMode = completion.completion_mode;
          debtBearing = completion.debt_bearing;
          completionWarnings = completion.warnings;
          verificationStatus = completion.verification_status;

          if (completionMode === CLEAN_COMPLETION) diskStatus = 'complete';
          else if (completionMode === DEBT_CARRYING_COMPLETION) diskStatus = 'complete_with_debt';
    ...
    // Check ROADMAP checkbox status without allowing stale checkmarks to override
    // debt-aware disk classification.
    ...
    phases.push({
      ...
      clean_completion: completionMode === CLEAN_COMPLETION,
      debt_bearing: debtBearing,
      verification_status: verificationStatus,
      completion_warnings: completionWarnings,
      roadmap_complete: roadmapComplete,
      checkbox_conflicts_with_disk: roadmapComplete && diskStatus !== 'complete',
    });
  }
  ...
  const currentPhase = phases.find(p =>
    ...
    || p.disk_status === 'executed'
    || p.disk_status === 'complete_with_debt'
  ) || null;
...
function cmdRoadmapUpdatePlanProgress(cwd, phaseNum, raw) {
  ...
  const roadmapStatus = summaryCount >= planCount && planCount > 0
    ? completionModeToRoadmapStatus(completionMode)
    : (summaryCount > 0 ? 'In Progress' : 'Planned');
  const shouldUseCompleteLanguage = completionModeUsesCompleteLanguage(completionMode);
  ...
  withPlanningLock(cwd, () => {
    ...
    roadmapContent = roadmapContent.replace(tableRowPattern, (fullRow) => {
      ...
      if (cells.length === 5) {
        ...
        cells[4] = isComplete && shouldUseCompleteLanguage ? dateField : '  ';
      } else if (cells.length === 4) {
        // 4-col: Phase | Plans | Status | Completed
        cells[1] = ` ${summaryCount}/${planCount} `;
        cells[2] = ` ${status.padEnd(11)}`;
        cells[3] = isComplete && shouldUseCompleteLanguage ? dateField : '  ';
      }
      return '|' + cells.join('|') + '|';
    });
    ...
    const planCountText = isComplete
      ? `${summaryCount}/${planCount} plans ${shouldUseCompleteLanguage ? 'complete' : 'executed'}`
      : `${summaryCount}/${planCount} plans executed`;
    roadmapContent = replaceInCurrentMilestone(roadmapContent, planCountPattern, `$1${planCountText}`);

    // Only clean completion should flip the roadmap checkbox.
    if (isComplete && shouldUseCompleteLanguage) {
  ...
  output({
    ...
    summary_count: summaryCount,
    status,
    completion_mode: isComplete ? completionMode : null,
    debt_bearing: isComplete ? completion.debt_bearing : false,
    clean_completion: isComplete ? shouldUseCompleteLanguage : false,
    complete: isComplete,
