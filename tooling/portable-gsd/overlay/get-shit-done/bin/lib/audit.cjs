...
function firstMeaningfulMarkdownLine(sectionBody, maxLength = 160) {
  if (!sectionBody) return '';
  const candidates = sectionBody
    .split('\n')
    .map(line => line.trim())
    .filter(Boolean)
  ...
  for (const candidate of candidates) {
    ...
    return sanitizeForDisplay(candidate.slice(0, maxLength));
  }
  return '';
}
...
function scanSeeds(planDir) {
  ...
  for (const entry of files) {
    ...
    const frontmatterSeedId = String(fm.id || '').trim();
    const seed_id = /^SEED-[\w-]+$/i.test(frontmatterSeedId)
      ? frontmatterSeedId
      : (seedIdMatch ? seedIdMatch[1] : path.basename(entry.name, '.md'));
    const slugSource = seedIdMatch ? seedIdMatch[1] : path.basename(entry.name, '.md');
    const slug = sanitizeForDisplay(slugSource.replace(/^SEED-/, ''));
    const seedContractVersion = String(fm.seed_contract_version || '').trim();
    const contractVintage = seedContractVersion
      ? sanitizeForDisplay(seedContractVersion)
      : 'legacy_unversioned';
    const whyThisMattersExcerpt = firstMeaningfulMarkdownLine(
      extractMarkdownSection(content, 'Why This Matters')
    );
    const strengtheningCarryExcerpt = firstMeaningfulMarkdownLine(
      extractMarkdownSection(content, 'Strengthening Carry')
    );
    const strengtheningCarryStatus = strengtheningCarryExcerpt ? 'present' : 'none';
    ...
    results.push({
      ...
      status: sanitizeForDisplay(status),
      title,
      contract_vintage: contractVintage,
      why_this_matters_excerpt: whyThisMattersExcerpt,
      strengthening_carry_status: strengtheningCarryStatus,
      strengthening_carry_excerpt: strengtheningCarryExcerpt,
    });
  }
...
function formatAuditReport(auditResult) {
  ...
  if (counts.seeds > 0) {
    ...
    for (const item of items.seeds.filter(i => !i.scan_error)) {
      const title = item.title ? ` — ${item.title}` : '';
      const vintage = item.contract_vintage ? ` [vintage: ${item.contract_vintage}]` : '';
      const strengthening = item.strengthening_carry_status ? ` [strengthening: ${item.strengthening_carry_status}]` : '';
      lines.push(`   • ${item.seed_id} [${item.status}]${title}${vintage}${strengthening}`);
      if (item.why_this_matters_excerpt) {
        lines.push(`     why: ${item.why_this_matters_excerpt}`);
      }
      if (item.strengthening_carry_excerpt) {
        lines.push(`     carry: ${item.strengthening_carry_excerpt}`);
      }
    }
  }
